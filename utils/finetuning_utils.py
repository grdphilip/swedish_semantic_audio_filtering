...
class DataCollatorSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features):
        ...
        for file_path in file_paths:
            ...
            input_features.append(self.processor.feature_extractor(audio_data, sampling_rate=sample_rate).input_features[0])
        
        batch = self.processor.feature_extractor.pad({"input_features":input_features}, return_tensors="pt")
        
        texts = [self.processor.tokenizer(sample["labels"]).input_ids for sample in features]
...
def create_dataset(manifest_filename):
    ...
    manifest_data = read_manifest_file(manifest_filename, raw=False)
    dataframe = pd.DataFrame(manifest_data)
    dataset = MyDataset(dataframe)
    return dataset


#Funkar med KB-labs modellen
