...
from utils.manifest_utils import convert_hf_dataset_to_manifest, read_manifest_file, write_manifest_file, remove_special_samples, convert_finetuning_manifest_to_filtering_manifest, convert_hf_dataset_to_manifest_eval
...
def main(args):
    ...
    os.makedirs("data/manifest_data/eval", exist_ok=True)
    

    # Load environment variables
    # Load dataset
    cv_dataset = load_dataset(args.dataset_name, args.dataset_language, split=args.split, token=args.token)
    # Will contain the columns audio, text, duration and whatever else is in the hf dataset
    
    
    # Convert dataset to manifest file
    convert_hf_dataset_to_manifest_eval(dataset=cv_dataset, dataset_type=args.dataset_type, manifest_filename=args.manifest_filename)
